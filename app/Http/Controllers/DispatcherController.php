...
class DispatcherController extends Controller
{
    ...
    {
        $Providers = new Drivers;

        if($request->has('latitude') && $request->has('longitude')) {
            $ActiveProviders = ProviderService2::AllAvailableServiceProvider($request->service_type)
                    ->get()
                    ->pluck('provider_id');
    ...
    public function assign($request_id, $provider_id)
    {
        try {
            ...
            $Provider = Drivers::findOrFail($provider_id);
            ...
            ProviderService2::where('provider_id',$Request->provider_id)->update(['status' =>'riding']);
            ...
            if(Auth::guard('admin')->user()){
                return redirect()
                        ...
                        ->with('flash_success', 'Request Assigned to Drivers!');
    ...
    public function store(Request $request) {
        ...
        try {
            $User = Users::where('mobile', $request->mobile)->firstOrFail();
        } catch (Exception $e) {
            try {
        ...
        if($request->has('schedule_time')){
            try {
                ...
                if($request->ajax()) {
                    ...
                    return redirect('dashboard')->with('flash_error', 'Already rides is Scheduled on this time.');
                }
        ...
        try{
            ...
            if($request->has('provider_auto_assign')) {
                $ActiveProviders = ProviderService2::AvailableServiceProvider($request->service_type)
                        ->get()
                        ->pluck('provider_id');
                ...
                Log::info('Assigned Drivers : ' . $UserRequest->current_provider_id);

                // Incoming rides push to provider
            ...
            if($request->ajax()) {
                ...
                return back()->with('flash_error', 'Something went wrong while sending rides. Please try again.');
            }
        }
    }
