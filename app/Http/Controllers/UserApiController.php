...
use App\Http\Controllers\DriverResources\TripController;
...
class UserApiController extends Controller
{
    ...
    {
        try {
            Users::where('id', $request->id)->update(['device_id'=> '', 'device_token' => '']);
    ...
    public function send_request(Request $request) {
        ...
        $this->validate($request, [
                ...
                'coupon_code' => 'exists:promocodes,coupon_code',
        ...
        Log::info('New Request from Users: '.Auth::user()->id);
        ...
        $Providers = Drivers::with('service')
        ...
        try{
            ...
            Users::where('id',Auth::user()->id)->update(['payment_mode' => $request->payment_mode]);
            ...
            if(in_array($UserRequest->status, ['SEARCHING','STARTED','ARRIVED','SCHEDULED'])) {
                ...
                if($UserRequest->status != 'SCHEDULED'){
                    ...
                    if($UserRequest->provider_id != 0){
                        ...
                        ProviderService2::where('provider_id',$UserRequest->provider_id)->update(['status' => 'active']);
    ...
    public function request_status_check() {
        ...
        try{
            ...
            $Timeout = Setting::get('driver_select_timeout', 180);
    ...
    public function rate_provider(Request $request) {
        ...
        try{
            ...
            Drivers::where('id',$UserRequest->provider_id)->update(['rating' => $average]);
    ...
    public function modifiy_request(Request $request) {
        ...
        try{
            ...
            if($request->ajax()){
                ...
                return redirect('dashboard')->with('flash_success', 'Users Changed Destination Address Successfully!');
            }
        } catch (Exception $e) {
            if($request->ajax()){
                return response()->json(['error' => trans('api.something_went_wrong')], 500);
    ...
    public function estimated_fare(Request $request){
        ...
        try{
            ...
            $ActiveProviders = ProviderService2::AvailableServiceProvider($request->service_type)->get()->pluck('provider_id');

            $distance = Setting::get('driver_search_radius', '10');
            $latitude = $request->s_latitude;
            $longitude = $request->s_longitude;

            $Providers = Drivers::whereIn('id', $ActiveProviders)
                ->where('status', 'approved')
    ...
    public function add_promocode(Request $request) {
        ...
        $this->validate($request, [
                'promocode' => 'required|exists:promocodes,coupon_code',
            ]);

        try{

            $find_promo = Promocode::where('coupon_code',$request->promocode)->first();
    ...
    public function show_providers(Request $request) {
        ...
        try{
            ...
            if($request->has('service')){
                ...
                $ActiveProviders = ProviderService2::AvailableServiceProvider($request->service)
                                    ->get()->pluck('provider_id');

                $Providers = Drivers::with('service')->whereIn('id', $ActiveProviders)
                    ->where('status', 'approved')
                    ->whereRaw("(1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance")
                    ->get();
                ...
                $ActiveProviders = ProviderService2::where('status', 'active')
                                    ->get()->pluck('provider_id');
    ...
    public function reset_password(Request $request){
        ...
        try{
            ...
            // $UpdatedAt = date_create($Users->updated_at);
