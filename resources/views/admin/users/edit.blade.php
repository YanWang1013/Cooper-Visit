            ...
            <form class="form-horizontal" action="{{route('admin.user.update', $user->id )}}" method="POST" enctype="multipart/form-data" role="form">
				...
				<div class="form-group row">
					...
					@if(isset($user->avatar))
                    	<img style="height: 90px; margin-bottom: 15px; border-radius:2em;" src="{{asset('storage/'.$user->avatar)}}">
                    @endif
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						<input class="form-control" type="number" value="{{ $user->phone_number }}" name="mobile" required id="mobile" placeholder="Mobile">
					</div>
				</div>
